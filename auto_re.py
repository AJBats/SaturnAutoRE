...
def cmd_integrate(config):
    ...
    if nop_candidates:
        ...
        if needs_experiment:
            ...
            print()
            print(f"  NOP tests are INTERACTIVE with the human — see nop-candidates")
            print(f"  for the full execution procedure.")
            print()
...
def cmd_nop_candidates(config):
    ...
    print()
    print(f"NOP tests are INTERACTIVE with the human. You cannot judge visual")
    print(f"behavior (rendering, car movement, camera, game state) from screenshots.")
    print(f"The human observes the emulator window and reports what they see.")
    print()
    ...
    print(f"Procedure:")
    print()
    print(f"  1. Tell the human what you're about to do and what to look for:")
    print(f"     \"I'm going to load save state [scenario], NOP the store at")
    print(f"      [patch_addr] in [function]. I expect [prediction].")
    print(f"      Please [play normally / hold throttle / etc] and tell me")
    print(f"      what you see.\"")
    print(f"  2. Load a save state:  load_state <scenario_path>")
    print(f"  3. Poke NOP (0x0009) over the store instruction:")
    print(f"     raw_command \"poke <patch_addr> 00 09\"")
    print(f"     (two separate bytes: 00 then 09 — poke takes individual bytes)")
    print(f"  4. Free-run. The human watches and reports what changed (or didn't).")
    print(f"  5. Reload the SAME save state (clean slate) to compare with/without")
    print()
